...
var callForm = function(form, index){
	...
	div[0].setAttribute('style', 'animation: div-collapse 2s .75s ease both');
	
	setTimeout(function(){
		div[0].setAttribute("style", 'animation: div-open 2s .75s ease both')
	}, 2250)
...
var closeForm = function(form){
	var formDiv = document.getElementsByClassName('mast__header');
	var welcome = document.getElementById('welcome');

	formDiv[0].setAttribute('style', 'animation: div-collapse 2s .75s ease both');
	
	setTimeout(function(){
		formDiv[0].setAttribute("style", 'animation: div-open 2s .75s ease both')
	}, 2250)
...
var backButtons = document.getElementsByClassName('fa-arrow-left');
...
for(var i = 0; i < backButtons.length; i++){
	backButtons[i].addEventListener('click', function(){
		if(this.parentNode.getAttribute("id") == 'login-form'){
			closeForm(document.getElementById('login-form'));
		} else {
			closeForm(document.getElementById('register-form'));
		}
	});
}
